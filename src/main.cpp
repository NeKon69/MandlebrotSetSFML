...
void main_thread() {
    ...
    sf::Font font;
    if (!font.openFromFile("fonts/LiberationMono-Bold.ttf")) {
    ...
    while (window.isOpen()) {
        ...
        while (const auto event = window.pollEvent()) {
            if (const auto _ = event->getIf<sf::Event::MouseWheelScrolled>() || event->is<sf::Event::MouseMoved>() || event->is<sf::Event::KeyPressed>() || event->is<sf::Event::MouseButtonPressed>()) {
